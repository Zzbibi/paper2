...
dataSizeArr = [650,700, 750, 800, 850, 900, 950, 1000]
len_dataSizeArr = len(dataSizeArr)
...
for r in range(round):
    ...
    for i in range(len_dataSizeArr):

        print('----------------------Data size:{}-----------------------'.format(dataSizeArr[i]))


        device = Device(numOfServer=8, numOfBank=3, numOfME=50, taskSize=[dataSizeArr[i], dataSizeArr[i]],
...
for i in range(5):
    temp = []
    for j in range(len_dataSizeArr):
        sum = 0
...
writer = pd.ExcelWriter(r'numberresult/datasize.xlsx')
