...
class Device(object):
    ...
    def getPoA(self):
        ...
        totalCostOfLocal = self.getTotalCostOfLocal()
        lowerBound = 0
        for i in range(1, self.numOfME + 1):
            min_costOnServer = 1000000000
            for s in range(1, self.numOfServer + 1):
                costOnServer = Device.LAMBDA_T * self.task[i][1] / self.computationPowerOfServer[s]
                costOnServer += (Device.LAMBDA_E * Device.SP + Device.LAMBDA_T) * self.task[i][0] / self.linkRate[i][s]
                if(min_costOnServer > costOnServer):
                    min_costOnServer = costOnServer
            min_costOnBank = 10000000000
            for m in range(1, self.numOfBank + 1):
                costOnBank = (1 + self.coEfficient[m][0] + self.coEfficient[m][1]) * self.task[i][1] * Device.RHO
                if(min_costOnBank > costOnBank):
                    min_costOnBank = costOnBank
            lowerBound += (min_costOnServer + min_costOnBank)

        return totalCostOfLocal / lowerBound


    def getInterestRateOfBank(self):
        for i in range(1, self.numOfBank + 1):
            num_loaner = len(self.loanerOfBank[i])
            self.interestRateOfBank[i] = self.coEfficient[i][0] / self.numOfME + self.coEfficient[i][1] * num_loaner
    ...
    def costOfLocal(self, i):
        ...
        #                                       2.8
        e = self.task[i][1] * math.pow(10, 9) * (1.3 * (self.computationPowerOfME[i] ** 2))  * math.pow(10, -9)
    ...
    def costOfOffloading(self, i, s, m):
        ...
        debt = (1 + self.coEfficient[m][0] / self.numOfME + self.coEfficient[m][1] * (num_loanerOfBank + 1)) * (self.task[i][1] * Device.RHO)
    ...
    def getPotentialFucntion(self):
        ...
        for i in range(1, self.numOfBank + 1):
            for j in range(1, len(self.loanerOfBank[i]) + 1):
                secondTerm += self.coEfficient[i][1] * Device.RHO * j
        thirdTerm = 0
        for i in range(1, self.numOfME + 1):
            if(sum(self.decision[i][0]) == 0):
                ...
                t_offload = self.task[i][0] / self.linkRate[i][s]
                e_offload = t_offload * Device.SP
                temp = self.coEfficient[m][0] / self.numOfME * (self.task[i][1] * Device.RHO)
                thirdTerm += (Device.LAMBDA_E * e_offload + Device.LAMBDA_T * t_offload + temp) / self.task[i][1]
        forthTerm = 0
        for i in range(1, self.numOfME + 1):
            if(sum(self.decision[i][0]) == 0):
                forthTerm += (self.costOfLocal(i) - (self.task[i][1] * Device.RHO)) / self.task[i][1]
    ...
    def goToNE(self, bank = True):
        ...
        # 计算beneficial ME的个数
        self.beneficialME.append(self.numOfBenificialME())

        return False


    def goToNE1(self, bank = True):
        '''达到NE返回True, 未达到返回False'''
        ...
        for i in range(1, self.numOfME + 1):
            ...
            if(len(updateAll) >= 2):
                ...
                update.append(updateAll[index])
            else:
                update.append(updateAll[0])
        # 如果没有需要更新的，NE已经达到
        if(len(update) == 0):
            return True
        else:
            # 根据cost进行排序
            update.sort(key = lambda x : x[2], reverse = True)
            # 选择使cost减小最多的更新（也即使potential function减小最多的）
            if(len(update) >= 5):
    ...
    def proposedAlgorithm1(self):
        while(self.goToNE1() == False):
            pass
        print('Iteration time is : {0}'.format(self.intertionTime))


    def allOffloading(self):
    ...
    def initPosition(self):
        ...
        for i in range(1, self.numOfME + 1):
            ...
            for j in range(1, self.numOfServer + 1):
                S = Device.DTP * 0.5 * 14 * (self.distance[i][j] ** -2.1) * math.pow(170, 2)
                rate = Device.BANDWIDTH * math.log(1 + S/N, 2) / (8 * 1024)
                self.linkRate[i].append(rate)



# device = Device(numOfServer = 8, numOfBank = 3, numOfME = 90, taskSize = [600, 1200],
...
# print('System cost of all offloading:{}'.format(device.getTotalCostOfAlgorithm()))
#
# device.restoreState()
